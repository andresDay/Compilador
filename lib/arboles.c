...
t_nodoa *obtener_nodo_con_hojas_mas_izq(t_nodoa *p_nodo, FILE *pf)
{
	...
	while (p_nodo != NULL)
	{
		...
		else if (strncmp(p_nodo->info.valor, "CUERPO", 6) == 0)
		{
			...
			{
				if (p_nodo->info.etiqueta_escrita == 0)
				{ //DEBO ESCRIBIR LA ETIQUETA DE CUERPO
		...
		if (es_padre_con_hojas(p_nodo->izq) == 0)
		{ //RETURNO EL HIJO IZQUIERDO
		...
		else if (es_padre_con_hojas(p_nodo->der) == 0)
